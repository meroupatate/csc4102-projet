...
\usecase{\haute}{Initialiser les serrures}
        %% précondition
        {chambre créée par l'administrateur $\land$ graine et sel utilisés pour la génération des clefs de la chambre obtenus auprès du système}
        %% postcondition
        {les deux clefs de la serrure sont initialisées}

\smallskip

\usecase{\haute}{Créer les nouveaux clients}
        %% précondition
        {première visite du client $\land$ informations personnelles obligatoires bien fournies par le client}
        %% postcondition
...
\priorityusecase{\moyenne}{Enregistrer l'occupation d'une chambre par un client}
